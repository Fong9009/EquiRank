...
import styles from '@/styles/pages/lender/fundedLoanDetails.module.css';

interface FundedLoanDetails {
  id: number;
  borrower_id: number;
...
interface FundedLoanDetailsProps {
  loanId: number;
  onClose: () => void;
}

export default function FundedLoanDetails({ loanId, onClose }: FundedLoanDetailsProps) {
  const { data: session } = useSession();
  const [loan, setLoan] = useState<FundedLoanDetails | null>(null);
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={`${styles.modal} ${backgroundColour}`} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={textColour}>Funded Loan Details</h2>
          <button onClick={onClose} className={styles.closeButton}>
            ×
          </button>
        </div>
        ...
        <div className={styles.content}>
          ...
          <div className={styles.amountSection}>
            ...
            <div className={styles.statusSection}>
              <span className={`${styles.status} ${styles.statusFunded}`}>
                Funded
              </span>
            </div>
          </div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.detailGrid}>
              ...
              <div className={styles.detailRow}>
                ...
                <span className={`${styles.value} ${textColour}`}>
                  <span className={`${styles.status} ${styles.statusFunded}`}>Funded</span>
                </span>
              </div>
            </div>
          </div>
